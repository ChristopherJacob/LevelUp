...
static esp_err_t i2c_init(void)
{
    ...
    ESP_LOGI(TAG, "i2c bus handle=%p", s_i2c_bus);
    return ESP_OK;
}
...
static esp_err_t expander_power_sequence(void)
{
    ...
    // This matches your demo. If this ever fails, we add an I2C scan and try other addr variants.
    ESP_RETURN_ON_ERROR(
        esp_io_expander_new_i2c_tca9554(I2C_PORT, ESP_IO_EXPANDER_I2C_TCA9554_ADDRESS_000, &ex),
        TAG, "tca9554 new failed");

    // Pins 0/1/2 as outputs, then toggle low -> high
    ESP_RETURN_ON_ERROR(
        esp_io_expander_set_dir(ex, IO_EXPANDER_PIN_NUM_0 | IO_EXPANDER_PIN_NUM_1 | IO_EXPANDER_PIN_NUM_2, IO_EXPANDER_OUTPUT),
...
esp_err_t display_fill_rgb565(uint16_t color)
{
    ...
    // Fill a line buffer and blast it repeatedly.
